  ...
  <div class="col-12">
    <div class="mb-3">
      ...
      <div class="mt-2">
        <div class="row">
          <div
            v-if="getDistributions.length > 0"
            class="col-12"
          >
            <div class="row mb-5">
              <div
                class="col-1 my-auto pr-0 text-right"
                @click="toggleDistributions()"
              >
                <span
                  v-if="!distributionsVisible"
                  ...
                  <span class="col-10">
                    <span class="row">
                      ...
                      <span class="col-md-5 col-12 mt-2 text-md-right text-left">
                        ...
                        >
                          <button
                            class="btn btn-sm btn-primary p-0 pl-2 w-100"
                            type="button"
                          ...
                          >
                            ...
                            <b-icon-chevron-down
                              scale="1.5"
                              class="material-icons small-icon float-right align-bottom"
                            >keyboard_arrow_down</b-icon-chevron-down>
                          </button>
                          <div class="dropdown-menu dropdown-menu-right bg-light">
                            ...
                            >
                              ...
                              <i
                                class="copy-text material-icons float-right align-bottom"
                            ...
                            <span
                              ...
                              class="dropdown-item d-block px-3 text-right"
                            >
                              <input
                                class="d-inline-block mr-2 py-0 px-1 w-75 small border border-secondary bg-white"
                                type="text"
                                :value="`${downloadURL}`"
                              >
                              <app-link
          ...
          <div class="col-10 offset-1">
            <div class="row">
              <span
                ...
                :key="i"
                class="col-4 col-sm-3 col-md-2 mt-md-0 mt-3 pr-0"
              >
          ...
          >
            <table class="table table-borderless table-responsive pl-3 bg-light">
              <tr v-if="!isEmpty(getPublisherInfo.name) && !isEmpty(getPublisherInfo.homepage)">
                ...
                <td>
                  <div v-if="has(getPublisherInfo, 'name') && !isNil(getPublisherInfo.name)">
                    {{ $t('message.metadata.name') }}: {{ getPublisherInfo.name }}
                  </div>
                  <div v-if="has(getPublisherInfo, 'homepage') && !isNil(getPublisherInfo.homepage)">
                    {{ $t('message.metadata.homepage') }}: {{ getPublisherInfo.homepage }}
                  </div>
                  <div v-if="has(getPublisherInfo, 'resource') && !isNil(getPublisherInfo.resource)">
                    {{ $t('message.metadata.resource') }}:
                    <app-link :to="getPublisherInfo.resource">
                      {{ getPublisherInfo.resource }}
                    </app-link>
                  </div>
                </td>
              </tr>
              <tr v-if="!isEmpty(getContactInfo.name) && !isEmpty(getContactInfo.email)">
                ...
                <td>
                  <div v-if="has(getContactInfo, 'name') && !isNil(getContactInfo.name)">
                    {{ $t('message.metadata.name') }}: {{ getContactInfo.name }}
                  </div>
                  <div v-if="has(getContactInfo, 'email') && !isNil(getContactInfo.email)">
                    {{ $t($t('message.metadata.email')) }}:
                    <app-link :to="`mailto:${getContactInfo.email}`">
                      {{ getContactInfo.email }}
                    </app-link>
                  </div>
                  <div v-if="has(getContactInfo, 'resource') && !isNil(getContactInfo.resource)">
                    {{ $t('message.metadata.resource') }}:
                    <app-link :to="getContactInfo.resource">
